...
function Write-Header($Message) {
    $width = [Math]::Max($Message.Length + 4, 60)
    $padding = [Math]::Floor(($width - $Message.Length) / 2)
    
    Write-Host "`n" -NoNewline
    Write-Host ("+" + "-" * ($width - 2) + "+") -ForegroundColor Yellow
    Write-Host ("|" + " " * ($padding - 1) + $Message + " " * ($width - $padding - $Message.Length - 1) + "|") -ForegroundColor Yellow
    Write-Host ("+" + "-" * ($width - 2) + "+") -ForegroundColor Yellow
...
function Write-ProgressBar {
    ...
    Write-Host "`r[$progressBar] $PercentComplete% - $Status" -NoNewline -ForegroundColor Cyan
    ...
    if ($PercentComplete -eq 100) {
        Write-Host "" # Nouvelle ligne a la fin
    }
}
...
function Show-InstallationProgress {
    ...
    $steps = @(
        @{Percent = 0; Status = "Preparation"},
        @{Percent = 20; Status = "Telechargement"},
        @{Percent = 50; Status = "Installation"},
        @{Percent = 80; Status = "Configuration"},
        @{Percent = 100; Status = "Termine"}
    )
    
    Write-ColorOutput "Yellow" "Installation de $Component..."
    
    # Simuler la progression pour le debut
    foreach ($step in $steps[0..1]) {
        Write-ProgressBar -PercentComplete $step.Percent -Status $step.Status -CurrentOperation $Component
        Start-Sleep -Milliseconds 200
    }
    
    # Executer l'action reelle
    & $InstallAction
    
    # Terminer la progression
    foreach ($step in $steps[2..4]) {
        Write-ProgressBar -PercentComplete $step.Percent -Status $step.Status -CurrentOperation $Component
        Start-Sleep -Milliseconds 200
    }
...
function Install-Dependencies {
    ...
    # Installation de rclone avec barre de progression
    if ($DryRun) {
        Write-ColorOutput "Cyan" "[DRY RUN] Installation de rclone simulee"
        Show-InstallationProgress -Component "rclone" -InstallAction {
        ...
        Show-InstallationProgress -Component "rclone" -InstallAction {
            try {
                ...
                Write-ColorOutput "Green" "`nrclone installe avec succes."
            } catch {
                Write-ColorOutput "Red" "`nErreur lors de l'installation de rclone: $($_.Exception.Message)"
                $script:ErrorCount++
            }
        }
    }
    
    # Installation de WinFSP avec barre de progression
    if ($DryRun) {
        Write-ColorOutput "Cyan" "[DRY RUN] Installation de WinFSP simulee"
        Show-InstallationProgress -Component "WinFSP" -InstallAction {
            Start-Sleep -Milliseconds 500
        }
    } else {
        Show-InstallationProgress -Component "WinFSP" -InstallAction {
            try {
                ...
                Write-ColorOutput "Green" "`nWinFSP installe avec succes."
            } catch {
                Write-ColorOutput "Red" "`nErreur lors de l'installation de WinFSP: $($_.Exception.Message)"
                $script:ErrorCount++
            }
        }
    }
    
    # Installation de NSSM avec barre de progression
    $nssmPath = "C:\nssm"
    if (!(Test-Path $nssmPath)) {
        if ($DryRun) {
            Write-ColorOutput "Cyan" "[DRY RUN] Installation de NSSM simulee"
            Show-InstallationProgress -Component "NSSM" -InstallAction {
                Start-Sleep -Milliseconds 500
            }
        } else {
            Show-InstallationProgress -Component "NSSM" -InstallAction {
                try {
                    ...
                    while (!$downloaded -and $retryCount -lt $maxRetries) {
                        try {
                            # Telechargement avec progression manuelle
                            ...
                            if ($retryCount -lt $maxRetries) {
                                Write-ProgressBar -PercentComplete 10 -Status "Nouvelle tentative" -CurrentOperation "NSSM ($retryCount/$maxRetries)"
                    ...
                    Write-ColorOutput "Green" "`nNSSM installe avec succes."
                } catch {
                    Write-ColorOutput "Red" "`nErreur lors de l'installation de NSSM: $($_.Exception.Message)"
                    $script:ErrorCount++
                }
            }
        }
